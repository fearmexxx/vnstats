...
import { OTHERS_SHARE } from '@/lib/constants';
import { Users, TrendingUp, Share2, Newspaper, Video, MessageCircle, Sparkles, ArrowRight, Settings } from 'lucide-react';
...
export default function DashboardClient({ firms, socialMetrics, marketAccounts }: Props) {
  ...
  const latestAccounts = marketAccounts[0]?.new_accounts || 0;
  const lastUpdated = marketAccounts[0]?.date || 'N/A';

  const pieData = [
    ...firms.map(f => ({ name: f.name, value: f.market_share })),
    { name: 'Others', value: OTHERS_SHARE }
  ];

  const tableData = firms.map(firm => {
    const social = socialMetrics.find(sm => sm.id === firm.id) || { facebook: 0, tiktok: 0, youtube: 0 };
    const estNewAccounts = Math.round((latestAccounts * (firm.market_share || 0)) / 100);
    return {
      ...firm,
      estNewAccounts,
  ...
  const growthData = [...marketAccounts].reverse().slice(-12).map(h => ({
    name: h.date.split('/').slice(1).join('/'),
    accounts: h.new_accounts
  }));
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 font-medium">Institutional Strategy Dashboard • Last updated: {lastUpdated}</p>
          </div>
          <div className="flex gap-3">
            <Link 
              href="/admin"
              className="flex items-center gap-2 px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-xl font-bold shadow-sm hover:bg-gray-50 transition-all"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard title="Total Social Reach" value="3.5M+" subValue="Followers" icon={<Share2 className="w-6 h-6 text-purple-600" />} />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <p className="text-xs text-gray-400 font-medium tracking-tight">Real-time follower data with WoW/MoM growth analysis</p>
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {tableData.map((firm, index) => (
                  <tr key={firm.id} className="group hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => router.push(`/firm/${firm.id}`)}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="text-sm font-bold text-gray-700">{firm.facebook.toLocaleString()}</div>
                      <div className={`text-[10px] font-black ${Number(firm.fbGrowth) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                        {Number(firm.fbGrowth) >= 0 ? '↑' : '↓'} {Math.abs(Number(firm.fbGrowth))}%
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="text-sm font-bold text-gray-700">{firm.tiktok.toLocaleString()}</div>
                      <div className={`text-[10px] font-black ${Number(firm.ttGrowth) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                        {Number(firm.ttGrowth) >= 0 ? '↑' : '↓'} {Math.abs(Number(firm.ttGrowth))}%
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="text-sm font-bold text-gray-700">{firm.youtube.toLocaleString()}</div>
                      <div className={`text-[10px] font-black ${Number(firm.ytGrowth) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                        {Number(firm.ytGrowth) >= 0 ? '↑' : '↓'} {Math.abs(Number(firm.ytGrowth))}%
                      </div>
                    </td>
